...
// ─── 7. getDouyinPostCommentReplies ────────────────────────────────

export async function getDouyinPostCommentReplies(
  videoId: string,
  commentId: string,
  options: { cursor?: string; limit?: number } = {},
): Promise<DouyinCommentListResult> {
  ...
  const result = await executeInPage(tabId, async (vId: string, comId: string, cur: string, lim: number) => {
    try {
      const apiUrl =
        ...
        '&comment_id=' + encodeURIComponent(comId) +
        '&cursor=' + encodeURIComponent(cur) +
        '&count=' + lim;
      const res = await fetch(apiUrl, { credentials: 'include' });
      if (!res.ok) {
        const errBody = await res.text().catch(() => '');
        return { error: 'douyin comment-replies failed: HTTP ' + res.status + ' — ' + errBody.slice(0, 200) };
      }
      const body = await res.text();
      if (!body) {
        return { error: 'douyin-signature-required: a-bogus token missing — endpoint /aweme/v1/web/comment/list/reply/ requires signed request' };
      }
      let data: any;
      try { data = JSON.parse(body); } catch {
        return { error: 'douyin comment-replies returned non-JSON body — likely anti-bot stub' };
      }
      const sc = typeof data.status_code === 'number' ? data.status_code : 0;
      if (sc !== 0) {
        if (sc === 8 || sc === 2154) return { error: 'douyin-not-logged-in: please sign in to www.douyin.com first' };
        return { error: 'douyin-signature-required: status_code=' + sc + ' — endpoint /aweme/v1/web/comment/list/reply/ requires signed request' };
      }
      const list: any[] = Array.isArray(data.comments) ? data.comments : [];
      const num = (x: any): number => (typeof x === 'number' ? x : (typeof x === 'string' ? (parseInt(x, 10) || 0) : 0));
      ...
      return { error: e?.message || 'douyin comment-replies scraper failed' };
    }
  }, [vid, cid, cursor, limit]) as DouyinCommentListResult | { error: string };
