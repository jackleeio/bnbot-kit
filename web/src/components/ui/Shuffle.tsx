...
const Shuffle = ({
  ...
  shuffleTimes?: number;
  animationMode?: 'evenodd' | 'all' | 'sliding';
  loop?: boolean;
  loopDelay?: number;
  stagger?: number;
  ...
  useGSAP(
    () => {
      ...
      const inners = () => wrappersRef.current.map((w: any) => w.firstElementChild).filter(Boolean);
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            if (scrambleCharset) randomizeScrambles();
            if (isVertical) gsap.set(strips, { y: (_i: number, t: any) => parseFloat(t.getAttribute('data-start-y') || '0') });
            else gsap.set(strips, { x: (_i: number, t: any) => parseFloat(t.getAttribute('data-start-x') || '0') });
            onShuffleComplete?.();
          },
          onComplete: () => {
            playingRef.current = false;
        ...
        const addTween = (targets: any[], at: number) => {
          ...
          if (isVertical) vars.y = (_i: number, t: any) => parseFloat(t.getAttribute('data-final-y') || '0');
          else vars.x = (_i: number, t: any) => parseFloat(t.getAttribute('data-final-x') || '0');
        ...
        } else if (animationMode === 'sliding') {
          // Overlapping windows of 3: [0,1,2], [2,3,4], [4,5,6]...
          const windowSize = 3;
          const step = 2; // overlap by 1
          const groups: any[][] = [];
          for (let i = 0; i < strips.length; i += step) {
            const group = strips.slice(i, i + windowSize);
            if (group.length) groups.push(group);
          }
          groups.forEach((group, gi) => {
            const startTime = gi * duration * 0.8;
            addTween(group, startTime);
          });
        } else {
    ...
    },
    {
      dependencies: [
        ...
        triggerOnHover, onShuffleComplete,
      ],
      scope: ref,
    },
  );

  const commonStyle = useMemo(() => ({ textAlign: textAlign as any, ...style }), [textAlign, style]);
