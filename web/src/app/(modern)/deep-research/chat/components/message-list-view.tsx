...
import { LoadingAnimation } from "~/components/deep-research/deer-flow/loading-animation";
import { Markdown } from "~/components/deep-research/deer-flow/markdown";
import { RainbowText } from "~/components/deep-research/deer-flow/rainbow-text";
import { RollingText } from "~/components/deep-research/deer-flow/rolling-text";
import {
  ScrollContainer,
  type ScrollContainerRef,
} from "~/components/deep-research/deer-flow/scroll-container";
...
import {
  ...
  CardHeader,
  CardTitle,
} from "~/components/deep-research/ui/card";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
} from "~/components/deep-research/ui/collapsible";
...
function MessageBubble({
  ...
  const hasLineBreak = (message.content ?? "").includes("\n");
  const isShortAssistantMessage =
    message.role === "assistant" && !hasLineBreak && normalizedContent.length <= 140;
  const showShadow =
    message.role === "user" || (message.role === "assistant" && !isShortAssistantMessage);
  ...
  return (
    <div
      className={cn(
        "group flex w-fit max-w-[85%] flex-col text-nowrap",
        isShortAssistantMessage ? "rounded-none px-0 py-0" : "rounded-2xl px-4 py-3",
        shadowClass,
        message.role === "user" && "bg-gray-100 text-black rounded-ee-none",
        message.role === "assistant" &&
          (isShortAssistantMessage
            ? "bg-transparent text-foreground"
            : "bg-card rounded-es-none"),
        className,
      )}
    >
      {children}
