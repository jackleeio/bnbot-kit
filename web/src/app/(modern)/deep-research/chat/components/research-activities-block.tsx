...
import { FavIcon } from "~/components/deep-research/deer-flow/fav-icon";
import Image from "~/components/deep-research/deer-flow/image";
...
import {
  ...
  AccordionItem,
  AccordionTrigger,
} from "~/components/deep-research/ui/accordion";
import { Skeleton } from "~/components/deep-research/ui/skeleton";
    ...
    <section className="mt-4 pl-4">
      <div className="font-medium italic">
        <RainbowText className="flex items-center" animated={searching}>
          <Search size={16} className={"mr-2"} />
          <span>Searching documents for&nbsp;</span>
          <span className="max-w-[500px] overflow-hidden text-ellipsis whitespace-nowrap">
  ...
  return (
    <>
      <div className="mt-4 font-medium italic">
        {hasError ? "Error Executing Code" : "Execution Output"}
      </div>
      <div className="bg-accent mt-2 max-h-[400px] max-w-[calc(100%-120px)] overflow-y-auto rounded-md p-2 text-sm">
        <SyntaxHighlighter
          language="plaintext"
  ...
  const toolDisplayName = useMemo(() => {
    const sourceName =
      ...
      !Array.isArray(toolCall.args)
        ...
        : undefined);
    if (sourceName) {
      return humanizeToolName(sourceName);
    }
    if (tool?.description) {
      return tool.description;
    }
    return "Tool Call";
  }, [tool?.description, tool?.name, toolCall.args, toolCall.name]);
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="item-1">
            ...
            <AccordionContent>
              ...
              {argsPreview && (
                <div className="bg-accent mb-4 max-h-[320px] max-w-[560px] overflow-y-auto rounded-md text-sm">
                  <SyntaxHighlighter
                    language="json"
                    style={resolvedTheme === "dark" ? dark : docco}
                    customStyle={{
